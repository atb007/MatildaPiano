...
{
    // Always use tail-off (release) when a note is on, including when the voice is stolen.
    // This avoids clicks and glitches when many notes trigger (e.g. swift mouse drag across keys).
    if (isNoteOn)
    {
        adsr.noteOff();
        // Leave isNoteOn true; renderNextBlock will call clearCurrentNote() when envelope finishes.
    }
    else
    {
        adsr.reset();
                                         ...
                                         int startSample, int numSamples)
{
    if (auto* playingSound = dynamic_cast<MatildaSamplerSound*>(getCurrentlyPlayingSound().get()))
    {
        ...
        // When release has finished, free the voice so it can be reused (e.g. for new or stolen notes).
